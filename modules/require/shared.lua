...
local function getModuleInfo(modpath, modname)
  ...
  if not modpath:find('^@') then
    ...
    while true do
      ...
      if di then
        if not di.short_src:find('^@cfx_lib/modules/require') and not di.short_src:find('^%[C%]') and not di.short_src:find('^citizen') and di.short_src ~= '?' then
          resourceSrc = di.source:gsub('^%@+([^/]+)/.+', '%1')
          break
        end
      else
        resourceSrc = cfx.cache.resource
        break
      end
    ...
    if modname and resourceSrc ~= cfx.cache.resource then
...
function cfx.load(filePath, env)
  local resourceSrc
...
function cfx.loadJson(filePath)
  local resourceSrc
  local modpath = filePath:gsub('%.', '/')
